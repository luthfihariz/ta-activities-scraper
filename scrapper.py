...
base_url = 'http://www.tripadvisor.ca/Attractions-'
location_url = 'g294226-Activities-Bali.html'
...
def main():
    ...
    with open('activities-bali.json', 'w', encoding='utf-8') as output:
        output.write(json.dumps(activities, indent=4))


def parse_review_page(url):
    dl_page_src(url, True)

    with open('tripadvisor-review.html', encoding='utf-8') as page_review_src:
        source = page_review_src.read()

    soup = BeautifulSoup(source, 'html.parser')

    categories = []
    categories_tag = soup.select(".separator .detail a")
    for category_tag in categories_tag:
        categories.append(category_tag.text.strip())

    review_section = soup.select("#REVIEWS")[0]

    review_tags = []
    review_tag_clouds = review_section.find_all("span", "ui_tagcloud fl")
    for review_tag_cloud in review_tag_clouds:
        review_tags.append(review_tag_cloud.text.strip())

    nearby_section = soup.select("#NEARBY_TAB")[0]
    map_container = nearby_section.select(".mapContainer")[0]
    latitude = map_container['data-lat']
    longitude = map_container['data-lng']

    photos = soup.select(".flexible_photo_wrap img")
    print(photos)


def dl_page_src(url, review_page=False):
    print(url)
    response = requests.get(url, headers=headers)
    soup = BeautifulSoup(response.content, 'html.parser')
    filename = 'tripadvisor-review.html' if review_page else 'tripadvisor.html'
    with open(filename, 'w', encoding='utf-8') as saved_page:
